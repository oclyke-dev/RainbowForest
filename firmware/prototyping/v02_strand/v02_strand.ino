...
for(uint8_t sensor = 0; sensor < 8; sensor++){
  ...
  // Turn on the sensor by writing a 0 value to the R channel
  leds[(2*sensor) + 0] = CRGB(0, 0, 0);

  // Turn on the LED to illuminate the subject area
  leds[(2*sensor) + 1] = CRGB(200, 200, 255);  // adjust the balance of white here... blue seems to need some help (higher Vf than other leds..)
  FastLED.show();
