...
SFE_ISL29125::SFE_ISL29125(TwoWire* i2c, uint8_t addr)
{
  _i2c = i2c;
  _addr = addr;
}
...
bool SFE_ISL29125::init()
{
  ...
  // // Start I2C
  // _i2c->begin(); // allow users to begin the sensor i2c port manually (b/c wire.begin re-assigns the pin functions)
...
//  _i2c->beginTransmission(_addr);
  _i2c->requestFrom(_addr, (uint8_t)2); // request 2 bytes of data
  data = _i2c->read();
  data |= (_i2c->read() << 8);
//  _i2c->endTransmission();
